(function () {
	...
	PinClusterer.prototype = {
	    ...
	    cluster: function (raves) {
			if (!this.loaded) return;
			if (!raves) {
			    if (!this._raves) return;
			} else {
			    this._raves = raves;
			}
			var self = this;
		...
		_redraw: function () {
			...
			this.layer.clear();	
			this.each(this._raves, this._addToClosestCluster);
			this.toMap();
			if (_defaults.debug && started) _log((new Date()) - started);
		},
		
		_addToClosestCluster: function (rave) {
			var distance 		= 40000,
				location = new mm.Location(rave.Latitude, rave.Longitude),
				clusterToAddTo 	= null,
				d;
			...
			if (clusterToAddTo && clusterToAddTo.containsWithinBorders(location)) {
				clusterToAddTo.add(rave);
			} else {
				this._createCluster(rave);
			}
		},
		
		_createCluster: function (rave) {
		...
		each: function (raves, fn) {
			if (!raves.length) return;
			for (var i = 0, rave; rave = raves[i]; i++) {
				var rslt = fn.apply(this, [rave, i]);
				if (rslt === false) break;
			}
		}
	...
	Cluster.prototype = {
		add: function (rave) {
		    var location = new mm.Location(rave.Latitude, rave.Longitude);
		...
		zoom: function () {
		    this._pinClusterer.map.setView({ bounds: Microsoft.Maps.LocationRect.fromLocations(this._locations) }); // TODO: If single rave, zoom closer
		},
	...
	Pin = function (location, cluster, options) {
		...
		this._create(cluster.raves);
	};
	
	Pin.prototype = {
		_create: function (raves) {
		    this.pushpin = new mm.Pushpin(this.location, this._options);
		    this.pushpin.raves = new Array();
		    this.pushpin.raves = raves;
			if (this._cluster.doClickToZoom) {
				var self = this;
				mm.Events.addHandler(this.pushpin, 'click', function () {
				    self._cluster.zoom();
				    if (!isMultipleVenuesInRaveCollection(self._cluster.raves)) {
				        killActivePopovers();
				        showEventInfoModal(getRaveIDsFromArray(self._cluster.raves));
				    }
				});
			}
		},
